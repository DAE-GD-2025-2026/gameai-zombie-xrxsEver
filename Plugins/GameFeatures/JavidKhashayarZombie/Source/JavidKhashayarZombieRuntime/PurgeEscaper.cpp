...
UPurgeEscaper::UPurgeEscaper()
{
	PrimaryComponentTick.bCanEverTick = true;
	PrimaryComponentTick.TickInterval = 0.1f;
}

void UPurgeEscaper::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
...
AAIController* UPurgeEscaper::GetAI() const
{
	APawn* Pawn = Cast<APawn>(GetOwner());
	return Pawn ? Cast<AAIController>(Pawn->GetController()) : nullptr;
}

float UPurgeEscaper::GetPurgeRadius(AActor* Zone) const
{
	if (!Zone)
		return 0.f;
...
float UPurgeEscaper::NearestEdgeDistance() const
{
...
bool UPurgeEscaper::IsInPurgeZone() const
{
	return bEscaping;
}

FVector UPurgeEscaper::OutwardDirection() const
{
	const AActor* Owner = GetOwner();
	UWorld* World = GetWorld();
	if (!Owner || !World)
...
bool UPurgeEscaper::PointIsSafe(const FVector& Point) const
{
	UWorld* World = GetWorld();
	if (!World)
		return false;
...
FVector UPurgeEscaper::FindSafeEscapePoint() const
{
...
void UPurgeEscaper::EscapePurge(float DeltaTime)
{
...
void UPurgeEscaper::StopEscaping()
{
