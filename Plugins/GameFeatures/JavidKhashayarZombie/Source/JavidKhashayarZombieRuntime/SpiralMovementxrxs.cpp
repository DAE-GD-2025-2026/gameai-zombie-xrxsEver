...
USpiralMovementxrxs::USpiralMovementxrxs()
{
...
void USpiralMovementxrxs::BeginPlay()
{
	Super::BeginPlay();
...
void USpiralMovementxrxs::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
...
void USpiralMovementxrxs::SetMovementPaused(bool bPaused)
{
...
void USpiralMovementxrxs::InitializeSpiralMovement()
{
	if (PawnOwner)
	{
...
FVector USpiralMovementxrxs::CalculateSpiralPosition(float Angle, float Radius)
{
...
void USpiralMovementxrxs::UpdateMovement(float DeltaTime)
{
	if (!PawnOwner)
	{
		return;
	...
	if (CurrentState == EMovementStatexrxs::Spiraling)
	{
	...
	else if (CurrentState == EMovementStatexrxs::MovingToHouse)
	{
		if (!CurrentHouseTarget)
		{
			CurrentState = EMovementStatexrxs::Spiraling;
			return;
		}
		...
		if (DistanceToHouse <= (StoppingDistance + 50.f) || CurrentPointReachTimer >= PointReachTimeout)
		{
			if (HouseTrackerComp)
			{
				...
				if (ExplorationPoints.Num() > 0)
				{
					...
					CurrentState = EMovementStatexrxs::ExploringHouse;
				}
				else
				{
	...
	else if (CurrentState == EMovementStatexrxs::ExploringHouse)
	{
		ExploreHouse(DeltaTime);
	}
}

void USpiralMovementxrxs::MoveToHouse(AActor* House)
{
	CurrentHouseTarget = House;
	CurrentState = EMovementStatexrxs::MovingToHouse;
	CurrentPointReachTimer = 0.0f;
}

void USpiralMovementxrxs::ExploreHouse(float DeltaTime)
{
...
void USpiralMovementxrxs::MoveToPoint(const FVector& TargetPoint, float DeltaTime, float AcceptanceRadius)
{
	if (!PawnOwner) return;
...
void USpiralMovementxrxs::UpdateFacingDirection(const FVector& Direction)
{
