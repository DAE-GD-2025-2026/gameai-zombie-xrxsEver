...
UItemHandlerxrxs::UItemHandlerxrxs()
{
	PrimaryComponentTick.bCanEverTick = true;
	PrimaryComponentTick.TickInterval = 0.25f;
}

void UItemHandlerxrxs::BeginPlay()
{
	Super::BeginPlay();
}

void UItemHandlerxrxs::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
...
UInventoryComponent* UItemHandlerxrxs::GetInventory() const
{
	return GetOwner() ? GetOwner()->FindComponentByClass<UInventoryComponent>() : nullptr;
}

ASurvivorPawn* UItemHandlerxrxs::GetSurvivor() const
{
	return Cast<ASurvivorPawn>(GetOwner());
}

bool UItemHandlerxrxs::IsConsumable(EItemType Type) const
{
	return Type == EItemType::Food || Type == EItemType::Medkit;
}

bool UItemHandlerxrxs::IsWeapon(EItemType Type) const
{
	return Type == EItemType::Pistol || Type == EItemType::Shotgun;
}

bool UItemHandlerxrxs::IsTrash(ABaseItem* Item) const
{
	return !Item || Item->GetItemType() == EItemType::Garbage || Item->GetValue() <= 0;
}

ABaseItem* UItemHandlerxrxs::SlotItem(int32 SlotIdx) const
{
...
int32 UItemHandlerxrxs::CountOfType(EItemType Type) const
{
	UInventoryComponent* Inv = GetInventory();
	if (!Inv)
		return 0;
...
bool UItemHandlerxrxs::HasType(EItemType Type) const
{
	return CountOfType(Type) > 0;
}

int32 UItemHandlerxrxs::LowestValueOfType(EItemType Type) const
{
	const int32 Slot = FindLowestValueSlotOfType(Type);
	return Slot >= 0 ? SlotItem(Slot)->GetValue() : -1;
}

int32 UItemHandlerxrxs::DesiredCount(EItemType Type) const
{
...
int32 UItemHandlerxrxs::FindLowestValueSlotOfType(EItemType Type) const
{
	UInventoryComponent* Inv = GetInventory();
	if (!Inv)
		return -1;
...
int32 UItemHandlerxrxs::FindFreeSlot() const
{
	UInventoryComponent* Inv = GetInventory();
	if (!Inv)
		return -1;
...
int32 UItemHandlerxrxs::FindGarbageOrSpentSlot() const
{
	UInventoryComponent* Inv = GetInventory();
	if (!Inv)
		return -1;
...
bool UItemHandlerxrxs::CanPlace(ABaseItem* Item) const
{
	if (!Item)
		return false;
...
bool UItemHandlerxrxs::IsDesirable(ABaseItem* Item) const
{
...
ABaseItem* UItemHandlerxrxs::GetBestItemToCollect() const
{
...
bool UItemHandlerxrxs::HasItemToCollect() const
{
	return GetBestItemToCollect() != nullptr;
}

ABaseItem* UItemHandlerxrxs::GetNearestWeapon(float Radius) const
{
	const AActor* Owner = GetOwner();
	UWorld* World = GetWorld();
	if (!Owner || !World)
...
bool UItemHandlerxrxs::UseIfBeneficial(int32 SlotIdx)
{
...
int32 UItemHandlerxrxs::MakeRoomFor(ABaseItem* Item)
{
	UInventoryComponent* Inv = GetInventory();
	if (!Inv || !Item)
		return -1;
...
bool UItemHandlerxrxs::TryCollect(ABaseItem* Item)
{
	UInventoryComponent* Inv = GetInventory();
...
void UItemHandlerxrxs::ManageInventory()
{
	UInventoryComponent* Inv = GetInventory();
	ASurvivorPawn* Survivor = GetSurvivor();
	if (!Inv || !Survivor)
