...
void UThreatHandler::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
...
AAIController* UThreatHandler::GetAI() const
{
	APawn* Pawn = Cast<APawn>(GetOwner());
	return Pawn ? Cast<AAIController>(Pawn->GetController()) : nullptr;
}

UInventoryComponent* UThreatHandler::GetInventory() const
{
	return GetOwner() ? GetOwner()->FindComponentByClass<UInventoryComponent>() : nullptr;
}

ABaseZombie* UThreatHandler::GetNearestZombie(float Radius) const
{
...
ABaseZombie* UThreatHandler::GetNearestThreat() const
{
	return GetNearestZombie(EngageExitRadius);
}

bool UThreatHandler::HasThreat() const
{
	return bEngaged;
}

FVector UThreatHandler::GetRepulsionDirection() const
{
	const AActor* Owner = GetOwner();
	UWorld* World = GetWorld();
	if (!Owner || !World)
...
bool UThreatHandler::HasLineOfSight(const AActor* Target) const
{
	const AActor* Owner = GetOwner();
	UWorld* World = GetWorld();
...
int32 UThreatHandler::FindWeaponSlotWithAmmo() const
{
...
void UThreatHandler::FaceInstant(const FVector& Direction)
{
...
void UThreatHandler::MoveAwayTo(const FVector& Direction)
{
...
void UThreatHandler::MoveToActorTracked(AActor* Goal, float AcceptanceRadius)
{
	AAIController* AI = GetAI();
	if (!AI || !Goal)
		return;
...
void UThreatHandler::MoveToPointTracked(const FVector& Point)
{
	AAIController* AI = GetAI();
	if (!AI)
		return;
...
void UThreatHandler::HoldPosition()
{
	AAIController* AI = GetAI();
	if (!AI)
		return;
...
void UThreatHandler::UpdateCombat(float DeltaTime)
{
...
int32 UThreatHandler::SlotOfWeaponWithAmmo(EItemType Type) const
{
	UInventoryComponent* Inv = GetInventory();
	if (!Inv)
		return -1;
...
void UThreatHandler::HandleArmed(float DeltaTime, ABaseZombie* Threat)
{
	ASurvivorPawn* Pawn = Cast<ASurvivorPawn>(GetOwner());
	if (!Pawn)
		return;
...
void UThreatHandler::HandleUnarmed(const FVector& DirToThreat, const FVector& Away)
{
	ASurvivorPawn* Pawn = Cast<ASurvivorPawn>(GetOwner());
	AAIController* AI = GetAI();
	if (!Pawn)
	...
	if (UItemHandler* Items = GetOwner()->FindComponentByClass<UItemHandler>())
	{
	...
	if (UHouseTracker* Houses = GetOwner()->FindComponentByClass<UHouseTracker>())
	{
...
void UThreatHandler::SetRunning(bool bRun)
{
	ASurvivorPawn* Pawn = Cast<ASurvivorPawn>(GetOwner());
	if (!Pawn)
		return;
